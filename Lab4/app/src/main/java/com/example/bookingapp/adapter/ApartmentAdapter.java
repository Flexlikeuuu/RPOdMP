...
public class ApartmentAdapter extends RecyclerView.Adapter<ApartmentAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder h, int p) {
        ...
        // ИСПРАВЛЕНО: Кнопка поделиться теперь работает
        h.btnShare.setOnClickListener(v -> {
            Intent intent = new Intent(Intent.ACTION_SEND);
            intent.setType("text/plain");
            intent.putExtra(Intent.EXTRA_TEXT, "Look at this: " + a.title + "\nPrice: " + a.price + "$");
            h.itemView.getContext().startActivity(Intent.createChooser(intent, "Share via"));
        });
    }
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        ImageView img;
        ImageButton btnEdit, btnDelete, btnBook, btnShare; // Добавлен btnShare
        ...
        ViewHolder(View v) {
            ...
            btnShare = v.findViewById(R.id.btnShare); // Добавлена инициализация
        }
    }
}